...
io.on('connection', (socket) => {
    ...
    socket.on('join', (options, callback) => {
        ...
        // socket.emit, io.emti, socket.brodcast.emit
        // io.to.emit emits message to a specific room
        // socket.broadcast.to.emit sends message to all clients in the room
    ...
    socket.on('sendLocation', (coords, callback) => {
        const user = getUser(socket.id)
        io.to(user.room).emit('locationMessage', generateLocatonMessage(user.username, `https://google.com/maps?q=${coords.latitude},${coords.longitude}`))
        callback()
    })
